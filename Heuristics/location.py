...
macs = ["00:0c:e7:4f:38:a5", "84:38:38:f6:58:40",
        "c0:ee:fb:72:0c:27", "18:dc:56:8c:27:56", "80:58:f8:d8:ad:e1"]
names = ["Micromax", "Samsung S5", "oneplus x", "Yureka", "Moto"]

# factor to convert to inches
# (doesn't have any effect on the nature of plots as everything is just scaled up)
factor = 1
...
aps_plot, = ax.plot([-22, 0, 0, -22], [1, 1, 24, 26],
...
validation_data1['Start_time'] = pd.to_datetime(
    validation_data1['Start_time'], infer_datetime_format=True)
validation_data1['Start_time'] = validation_data1['Start_time'].apply(
    lambda x: x + datetime.timedelta(hours=12))  # todo write proper code
validation_data1['Start_time'] = validation_data1['Start_time'].apply(
...
for file in f:
    ...
    if(ext == ".log"):
        print(file)
        df = pd.read_json("../spencers_data/%s" %
                          (file), lines=True)  # reading data
        df['ts'] = pd.to_datetime(df['ts'], infer_datetime_format=True)

        # converrting utc time ist
        df['ts'] = df['ts'].apply(
            lambda x: x + datetime.timedelta(hours=5, minutes=30))

        # filtering out data corresponding to our macs
        df = df.loc[df['mac'].isin([mac])]

        # making code granular by per minute
        df['ts'] = df['ts'].apply(lambda x: x.strftime('%H:%M'))
        df = df.groupby(
            ['nasid', 'controllerid', 'position', 'ts', 'mac']).mean()
        df.reset_index(inplace=True)

        df = df.groupby(['nasid', 'position', 'ts', 'mac'])
        lst = list(df)
        df_loc_track = pd.DataFrame(
            columns=['nasid', 'position', 'ts', 'mac',
                     'controllerid', 'pwr', 'count'],
            dtype=object
        )
        ...
        for i in range(len(lst)):
            ...
            df_loc_track.loc[i, :] = [x[0][0], x[0][1], x[0][2], x[0][3], " ".join(
                cid_list), " ".join(pwr_list), len(cid_list)]
