...
df_group_ap = df.groupby(['mac', 'ts'])
lst = list(df_group_ap)
df_loc_track = pd.DataFrame(columns=['mac', 'ts', 'controllerid', 'pwr', 'count'])
count = 0
for i in range(len(lst)):
    ...
    print(cid_list)
    print(pwr_list)
    df_loc_track.loc[count, :] = [x[0][1], x[0][0], str(cid_list), str(pwr_list), len(cid_list)]
    count += 1

grp = df_loc_track.groupby(['mac'])
lst = list(grp)
observed_sequences = []
observed_timestamps = []
...
for i in range(len(macs)):
    ...
    thefile = open('hidden_states_model_all_device_'+names[i]+'.txt', 'w')
